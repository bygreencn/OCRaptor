package mj.ocraptor.file_handler.filter;
...
public enum FileType {
  ...
  CODE_JAVA("text/x-java:java"), CODE_HTML("text/plain:html:htm:xhtml"), CODE_XHTML(
      "application/xhtml+xml:html:htm:xhtml"), CODE_XHTML2("application/xml:html:htm:xhtml"), CODE_BASH(
      "application/x-shellscript:sh"), CODE_PEARL("application/x-perl:pl"), CODE_CMD(
      "text/x-csrc:cmd"), CODE_XSL("application/xslt+xml:xsl"), CODE_XML("application/xml:xml"), CODE_CPP(
      "text/x-c++src:cpp"), CODE_HPP("text/x-c++hdr:hpp"), CODE_PYTHON("text/x-python:py"), CODE_PHP(
      "application/x-php:php"), CODE_CSS("text/css:css"), CODE_JAVASCRIPT(
   ...
   * @return
   */
  public static boolean isValidTextFile(final File file) {
    final Config cfg = Config.inst();
    final String[] textExtensions = cfg.getProp(ConfigString.TEXT_FILE_EXTENSIONS).split(";");
    return isValidTextFile(file, textExtensions);
  }
   ...
   * @return
   */
  public static boolean isValidTextFile(final File file, final String[] extensions) {
    if (extensions != null) {
      final String fileExtension = FilenameUtils.getExtension(file.getName()).toLowerCase();
      for (final String extensionToCheck : extensions) {
  ...
  public static boolean isValidXmlFile(final File file) {
    final Config cfg = Config.inst();
    final String[] xmlExtensions = cfg.getProp(ConfigString.XML_FILE_EXTENSIONS).split(";");
    return isValidXmlFile(file, xmlExtensions);
  }
   ...
   * @return
   */
  public static boolean isValidXmlFile(final File file, final String[] extensions) {
    if (extensions != null) {
      final String fileExtension = FilenameUtils.getExtension(file.getName()).toLowerCase();
      for (final String extensionToCheck : extensions) {
        if (fileExtension.equals(extensionToCheck.trim())) {
