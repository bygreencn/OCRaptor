...
public class ImageTextExtractorTess4j extends ImageTextExtractor {
  ...
  private String ocr(File file, String originalPath, String language) {
    ...
    if (command != null && !command.trim().isEmpty()) {
      ...
      while (!Thread.currentThread().isInterrupted() && executorThread.isAlive()) {
        ...
        if (!pause) {
          pause = this.cfg.isPaused();
        }

        // monitoring, if timeout occures --> kill process
        try {
          Thread.sleep(CHECK_TIMEOUT_INTERVAL);
          final int timeout = this.cfg.getProp(ConfigInteger.OCR_PROCESSING_TIMEOUT_IN_SECONDS)
              * ONE_SECOND_IN_MS;
          if (duration > timeout || this.cfg.isShutdown() || pause) {
      ...
      if (pause) {
        while (!Thread.currentThread().isInterrupted() && !this.cfg.isShutdown()) {
          try {
            Thread.sleep(100);
          } catch (InterruptedException e) {
            e.printStackTrace();
  ...
  private String extractText(BufferedImage image, File file, String originalPath,
    ...
    try {
      ...
      languageStrings = StringTools.removeLastCharacters(languageStrings, 1);
